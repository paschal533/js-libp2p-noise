 ...
 *   - Protocol ID is /noise-mlkem768-hfs/0.2.0
...
describe('NoiseHFS (post-quantum ConnectionEncrypter)', () => {
  ...
  describe('construction', () => {
    it('protocol ID is /noise-mlkem768-hfs/0.2.0', () => {
      const n = new NoiseHFS(makeComponents(localPeer))
      expect(n.protocol).to.equal('/noise-mlkem768-hfs/0.2.0')
    })
    ...
    it('noiseHFS factory returns a NoiseHFS instance', () => {
      ...
      expect(instance.protocol).to.equal('/noise-mlkem768-hfs/0.2.0')
    })
  ...
  describe('protocol isolation', () => {
    it('two NoiseHFS connections have protocol /noise-mlkem768-hfs/0.2.0, not /noise', () => {
      const init = new NoiseHFS(makeComponents(localPeer))
      const resp = new NoiseHFS(makeComponents(remotePeer))
      expect(init.protocol).to.equal('/noise-mlkem768-hfs/0.2.0')
      expect(resp.protocol).to.equal('/noise-mlkem768-hfs/0.2.0')
    ...
    it('NoiseHFS and classical Noise have different protocol strings', async () => {
      ...
      expect(pqNoise.protocol).to.equal('/noise-mlkem768-hfs/0.2.0')
